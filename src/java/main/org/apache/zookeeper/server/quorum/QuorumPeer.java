...
public class QuorumPeer extends Thread implements QuorumStats.Provider {
    ...
    public synchronized void start() {
        // 恢复快照数据
        loadDataBase();
        // 启动zkServer，相当于用户可以通过2181这个口进行通信了
        cnxnFactory.start();
        // 启动leader的选举
        startLeaderElection();
        // 调用thread.start方法，执行QuorumPeer.run();
        super.start();
    }
    ...
    synchronized public void startLeaderElection() {
        ...
        /**
         * getView().values() 相当于获取的zoo.cfg中配置的servers地址
    ...
    protected Election createElectionAlgorithm(int electionAlgorithm){
        ...
        case 3:
            ...
            if(listener != null){
                listener.start(); // 启动listener，用于监听其他机器发送过来的请求
                le = new FastLeaderElection(this, qcm); // 选举算法
            } else {
    ...
    public void run() {
        ...
        try {
            ...
            while (running) {
                switch (getPeerState()) {
                    // 如果你当前是刚刚才启动的话，你的状态都是LOOKING状态，处于寻找leader的过程中；
                    // 如果没有leader，此时你就要发起投票给其他所有人尝试去选举leader；
                    // 如果已经有leader，此时你就让自己作为follower即可；
                case LOOKING:
                    LOG.info("LOOKING");

                    if (Boolean.getBoolean("readonlymode.enabled")) {
                        /**
                         * 是否设置只读模式，默认false，所以默认不会进去该方法
                         */
                        ...
                        try {
                            setBCVote(null);
                            // makeLEStrategy().lookForLeader()，leader选举，会卡住直到选举出leader；
                            setCurrentVote(makeLEStrategy().lookForLeader());
                        } catch (Exception e) {
                            LOG.warn("Unexpected exception", e);
                            setPeerState(ServerState.LOOKING);
