...
public class Learner {       
    ...
    void writePacket(QuorumPacket pp, boolean flush) throws IOException {
        synchronized (leaderOs) {
            if (pp != null) {
                // 基于leader的jute输出流，将QuorumPacket这个对象进行序列化之后，以字节流发送到leader那儿去了；
    ...
    throws IOException, ConnectException, InterruptedException {
        ...
        for (int tries = 0; tries < 5; tries++) { // 最多重试五次
            try {
                sock.connect(addr, self.tickTime * self.syncLimit); // 走tcp三次握手，完成连接的建立
                sock.setTcpNoDelay(nodelay); // 如果连接建立，双方就有一个socket进行通信
                break;
            } catch (IOException e) {
                if (tries == 4) {
        ...
        // 获得对leader的输入流 -> 缓冲输入流，最后封装成Archive缓冲输入流（leaderIs）；
        leaderIs = BinaryInputArchive.getArchive(new BufferedInputStream(
                sock.getInputStream()));

        // 缓冲输出流，然后封装成Archive缓冲输出流（leaderOs）
    ...
    protected long registerWithLeader(int pktType) throws IOException{
        ...
        qp.setType(pktType);
        qp.setZxid(ZxidUtils.makeZxid(self.getAcceptedEpoch(), 0)); // 我最大的zxid
        ...
        // 先讲learnerInfo这个对象进行序列化，写入底层的ByteArrayOutputStream
        boa.writeRecord(li, "LearnerInfo"); // li.serialize(boa, "LearnerInfo");
        // 将序列化之后的learnerInfo对象转化为字节数组，设置到了QuorumPacket对象中去
        qp.setData(bsid.toByteArray());

        // 发送follower的注册信息到leader
        writePacket(qp, true); // qp.serialize(leaderOs, "packet");
        // 读取leader发送回来的leader信息
        readPacket(qp);
