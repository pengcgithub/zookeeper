...
public class LearnerHandler extends Thread {
    ...
    public void run() {
        try {
            ...
            // 收到follower的注册请求之后，再向follower返回一个响应；响应包含leader相关的信息；
            ...
            // todo 暂时不看，follower和leader刚建立连接，会进行数据同步
            ...
            /**
             * 把数据同步完之后，这边会单独启动一个线程；
             * 额外启动的线程，主要工作后续应该是给follower不停的同步数据；
             */
            new Thread() {
                public void run() {
