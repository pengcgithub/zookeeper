...
public class Leader {
    ...
    /**
     * leader的局部变量，初始化leader的时候，就会初始化ServerSocket变量；
     */
    ServerSocket ss;
    ...
    Leader(QuorumPeer self,LeaderZooKeeperServer zk) throws IOException {
        ...
        try {
            ...
            if (!self.getQuorumListenOnAllIPs()) {
                ...
                // 2888：leader和follower之间相互通信的接口
                // 3888：用户leader选举时监听的端口
        ...
        public void run() {
            try {
                while (!stop) {
                    try{
                        /**
                         * 阻塞住，ss变量是在leader初始化的时候建立的；
                         * 如果有follower进行了连接，那么这边会拿到连接的socket，继续往下执行；
                         *
                         * 换言之，下面的逻辑是，拿到一个follower的连接，然后对这个连接创建对应的LearnerHandler线程，负责与这个follower之间通信；
                         */
                        Socket s = ss.accept();
    ...
    void lead() throws IOException, InterruptedException {
        ...
        try {
            ...
            // new followers.
            // 启动LearnerCnxAcceptor，等待follower和其建立连接的
