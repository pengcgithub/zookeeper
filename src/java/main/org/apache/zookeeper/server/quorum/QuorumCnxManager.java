...
public class QuorumCnxManager {
    ...
    public boolean initiateConnection(Socket sock, Long sid) {
        ...
        if (sid > self.getId()) {
            // 不被允许和比自己大的机器主动建立连接，所以需要将刚刚建立好的连接给关闭掉；
    ...
    public void toSend(Long sid, ByteBuffer b) {
        ...
        if (self.getId() == sid) {
             ...
             */
             ...
             // 这个时候投票消息未必能够发送出去，因为当前节点与外部节点的通道是否已建立还未知，所以继续执行 connectOne
             connectOne(sid);
     ...
     *
     * QuorumCnxManager.Listener 线程处理逻辑如下：
     * 1.监听当前节点的 election address 等待接收外部节点连接
     * 2.读取外部节点的 server id 并与当前节点的 server id 比较；若前者小则关闭连接，改由当前节点发起连接
     * 3.反之为外部节点分配 SendWorker，RecvWorker 线程及消息发送队列
     */
        ...
        public void run() {
            ...
            while((!shutdown) && (numRetries < 3)){
                try {
                    ...
                    ss.bind(addr);
                    while (!shutdown) {
                        Socket client = ss.accept(); // BIO监听连接请求，并处理
                        setSockOpts(client);
